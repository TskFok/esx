...
- SSH 认证前会校验已保存的主机指纹；缺少指纹的旧记录需要重新验证并保存。通过 SSH 执行请求时暂不支持自定义 CA 或证书指纹模式，选择这些模式会明确报错。
...
- AI API Key 与服务的协议、主机和端口一起保存在钥匙串中；更换服务地址后需要为新服务填写密钥。
- AI 功能默认关闭，且不会主动连接 Elasticsearch，仅处理请求文本本身。

### 7. 服务器状态页

- 用概览 / 运维 / 索引三个标签展示集群健康、节点指标和索引列表。
- 只在打开对应标签时请求该数据，并缓存 30 秒；再次进入未过期标签不重复请求。
- 刷新按钮只刷新当前标签。
...
- 连接地址不接受内嵌账号密码；Console 请求只能发送到当前连接的同源地址。旧配置和日志中的 URL 凭据会在加载时清理。
- 整体设计偏向本地单机使用，不依赖服务端。
...
在 Console 侧边栏进入「服务器状态」，页面分为概览、运维、索引三个标签：

- **概览**：集群健康与分片分布
- **运维**：CPU、heap、磁盘水位等节点指标
- **索引**：索引搜索与多列排序

数据按需请求：只有打开对应标签时才拉取，并缓存 30 秒；再次进入未过期标签不会重复请求。刷新按钮仅刷新当前标签。

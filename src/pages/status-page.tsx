...
function HealthDistribution({ status }: { status: ServerStatusSnapshot }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">健康分布</p>
          <p className="mt-2 text-sm text-slate-500">按 index 健康状态统计</p>
        </div>
        <span className={cn("rounded-full border px-3 py-1 text-xs font-bold", healthBadgeClasses[status.cluster.health])}>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
        {segments.map((segment) => (
          <div key={segment.health} className="flex items-center gap-2 rounded-xl bg-slate-50 px-3 py-2">
            <span className={cn("h-2.5 w-2.5 rounded-full", healthDotClasses[segment.health])} />
...
function ShardOverview({ status }: { status: ServerStatusSnapshot }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">分片状态</p>
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
        <p className="rounded-xl bg-emerald-50 px-3 py-2 text-emerald-700">Started {shardCounts.started}</p>
        <p className="rounded-xl bg-cyan-50 px-3 py-2 text-cyan-700">Relocating {shardCounts.relocating}</p>
        <p className="rounded-xl bg-amber-50 px-3 py-2 text-amber-700">Initializing {shardCounts.initializing}</p>
        <p className="rounded-xl bg-rose-50 px-3 py-2 text-rose-700">Unassigned {shardCounts.unassigned}</p>
      </div>
...
export function StatusPage() {
  ...
  return (
    <div className="min-h-screen bg-hero-grid px-4 py-6 sm:px-8" onContextMenu={(event) => event.preventDefault()}>
      <div className="mx-auto max-w-7xl space-y-6">
        <Card className="p-6 sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              <p className="text-xs uppercase tracking-[0.28em] text-emerald-600">ESX Status</p>
              <div className="mt-3 flex flex-wrap items-center gap-3">
                <h1 className="text-3xl font-extrabold text-slate-950">服务器状态</h1>
                {status ? (
                  <span className={cn("rounded-full border px-3 py-1 text-xs font-bold", healthBadgeClasses[status.cluster.health])}>
                    {healthLabels[status.cluster.health]}
                  </span>
          ...
          {statusError ? (
            <div className="mt-6 flex items-start gap-3 rounded-2xl border border-rose-100 bg-rose-50 p-4 text-sm leading-6 text-rose-700">
          ...
          {status?.partialFailures.length ? (
            <div className="mt-6 flex items-start gap-3 rounded-2xl border border-amber-100 bg-amber-50 p-4 text-sm leading-6 text-amber-800">
              <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
              <div>
                <p className="font-bold">分片状态不完整</p>
                <p className="mt-1">{status.partialFailures[0]}</p>
              </div>
            </div>
          ) : null}
        </Card>
        ...
        {loading && !status ? (
          <Card className="flex min-h-[340px] items-center justify-center p-8">
            <div className="text-center">
              <Loader2 className="mx-auto h-8 w-8 animate-spin text-emerald-600" />
              <p className="mt-4 text-sm font-semibold text-slate-700">正在读取集群、索引和分片状态...</p>
            </div>
          </Card>
        ) : status ? (
          <>
            ...
            <Card className="overflow-hidden p-0">
              <div className="border-b border-slate-100 p-5 sm:p-6">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    <p className="text-xs uppercase tracking-[0.22em] text-emerald-600">Indices</p>
                    <h2 className="mt-2 text-2xl font-extrabold text-slate-950">Index 数据与状态</h2>
                ...
                <table className="min-w-[980px] w-full border-collapse text-left">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="px-5 py-3 text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Pri/Rep</th>
                      <th className="px-5 py-3">
                        <SortHeader label="Docs" sortKey="docs" currentSort={sort} onChange={changeSort} />
                      </th>
                      <th className="px-5 py-3 text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Deleted</th>
                      <th className="px-5 py-3">
                        <SortHeader label="Store" sortKey="store" currentSort={sort} onChange={changeSort} />
                      </th>
                      <th className="px-5 py-3 text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Shards</th>
                    </tr>
                  </thead>
                        ...
                        <tr key={index.name} className="align-top hover:bg-slate-50/80">
                          <td className="max-w-[320px] px-5 py-4">
                            <div className="flex min-w-0 items-start gap-3">
                              <Activity className={cn("mt-0.5 h-4 w-4 shrink-0", healthTextClasses[index.health])} />
                              <div className="min-w-0">
                                <p className="break-all text-sm font-bold text-slate-950">{index.name}</p>
                                {index.name.startsWith(".") ? (
                                  <p className="mt-1 text-xs text-slate-500">系统/隐藏 index</p>
                                ) : null}
                              </div>
                            </div>
                          </td>
                          <td className="px-5 py-4">
                            <span className={cn("rounded-full border px-2.5 py-1 text-xs font-bold", healthBadgeClasses[index.health])}>
                              {index.health}
                            </span>
                          </td>
                          <td className="px-5 py-4 text-sm font-semibold text-slate-700">{index.status}</td>
                          <td className="px-5 py-4 text-sm text-slate-600">
                            {formatNumber(index.primaryShards)} / {formatNumber(index.replicaShards)}
                          </td>
                          <td className="px-5 py-4">
                            <p className="text-sm font-semibold text-slate-900">{formatNumber(index.docsCount)}</p>
                            <Meter value={index.docsCount} max={displayedStats.maxDocs} tone="docs" />
                          </td>
                          <td className="px-5 py-4 text-sm text-slate-600">{formatNumber(index.docsDeleted)}</td>
                          <td className="px-5 py-4">
                            <p className="text-sm font-semibold text-slate-900">{formatDataBytes(index.storeBytes)}</p>
          ...
          <Card className="p-8 text-center">
            <p className="text-base font-bold text-slate-900">还没有服务器状态数据</p>
            <p className="mt-2 text-sm leading-7 text-slate-500">点击刷新状态重新读取当前连接。</p>
          </Card>
        )}
      </div>
    </div>
